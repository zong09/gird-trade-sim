...
- **Order fills (crossing-based)**: a buy fills only on a downward cross (`prevRef > level && low <= level`), a sell only on an upward cross (`prevRef < level && high >= level`), where `prevRef` is the previous candle's close. Levels the price never traded through are never filled — prevents phantom inventory bought at untraded prices (the bug fixed on `feature/fix-backtest-logic`).

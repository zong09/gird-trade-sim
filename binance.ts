...
// Download+extract one month's CSV into cacheDir, return its path (or null on 404).
function ensureMonthCsv(symbol: string, interval: string, month: string): { path: string; downloaded: boolean } | null {
  ...
  const csvPath = path.join(cacheDir, `${base}.csv`);
  ...
  try {
    ...
    execFileSync('unzip', ['-o', zipPath, '-d', cacheDir], { stdio: ['ignore', 'ignore', 'ignore'] });
